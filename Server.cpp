...
void Server::_hundleMessage(string message, int clientFd)
{
	...
	while (iss >> command)
	{
		...
		else if (command == "USER")
		{
			...
			if (userName.empty())
			{
				string msg = ":0.facebook 461 " + user->getNickName() + " USER :Not enough parameters\r\n";
		...
		else if (command == "PRIVMSG")
		{
			...
			if (recipent[0] == '#')
			{
				if (_channels.find(recipent) != _channels.end())
				{
					...
					{
						User *userr = *it;
						if (userr->getNickName() != user->getNickName())
						{
							string message = ":" + userr->getNickName() + "!" + userr->getUserName() + "@" + "0.0.0.0" + ".IP PRIVMSG " + recipent + " " + msg + "\r\n";
							send(userr->getFd(), message.c_str(), message.size(), 0);
						}
					}
				}
			}
				...
				while (it != _users.end())
				{
					...
					if(use->getNickName() == recipent)
					{
						...
						if (mes[0] == ' ' && mes[1] == ':')
							mes = mes.substr(2);
						cout << "PRIV " << mes << endl;
						string prvMsg = ":" + user->getNickName() + "!" + user->getUserName() + "@" + "0.0.0.0" + ".IP PRIVMSG " + recipent + " " +  mes + "\r\n";
		...
		else if (command == "MODE")
		{
			...
			if (_channels.find(channelName) != _channels.end())
			{
				...
				if(mode.empty())
				{
					reponse = ":0.Facebook 324 " + user->getNickName() + " " + channelName + " +oitlk\r\n";
					send(clientFd, reponse.c_str(), reponse.size(), 0);
					std::time_t creationTime = channel.getCreationTime();
					string time = std::to_string(creationTime);
					reponse = ":0.Facebook 329 " + user->getNickName() + " " + channelName + " " + time + " \r\n";
					send(clientFd, reponse.c_str(), reponse.size(), 0);
				}
				else if(mode == "+i")
				{
					...
					string msg = ":" + user->getNickName() + "!" + user->getUserName() + "@0.0.0.0.IP MODE " + channelName + " +k " + argument + "\r\n";
					send(clientFd, msg.c_str(), msg.size(), 0);
				}
				else if(mode == "-k"){
					channel.setHasPass(false);
					reponse = ":" + user->getNickName() + "!" + user->getUserName() + "@" + "0.0.0.0.IP MODE " + channelName + " -k\r\n";
					send(clientFd, reponse.c_str(), reponse.size(), 0);
					channel.setPass("");
				}
				else if(mode == "+o")
